...
const savePsiPoinCostValueForPsiAssault = psiPointCostInput.value
        ...
        console.log(skillIndex, selectedPsiDisciplineObj[0].psiSchool)
        ...
        function checkAllActiveBuffs(name) {
            ...
            for (let i = 0; i < allActiveBuffs.length; i++) {
                if (allActiveBuffs[i].innerText == '' || allActiveBuffs[i].innerText.includes('folyamatos')) {
                    if (selectedPsiDisciplineObj[0].psiDiscName == "Fájdalomtűrés") {
                        ...
                        break
                    } else if (selectedPsiDisciplineObj[0].psiDiscName == "Chi-harc" && checkAllActiveBuffs('Chi')==false) {
                ...
                charAtk.value = parseFloat(charAtk.value) + parseFloat(selectedPsiDisciplineObj[0].benefit[skillIndex - 1].atkAndDef)
                charDef.value = parseFloat(charDef.value) + parseFloat(selectedPsiDisciplineObj[0].benefit[skillIndex - 1].atkAndDef)
                charDefWithParry.value = parseFloat(charDefWithParry.value) + parseFloat(selectedPsiDisciplineObj[0].benefit[skillIndex - 1].atkAndDef)
                charDefWithEvasion.value = parseFloat(charDefWithEvasion.value) + parseFloat(selectedPsiDisciplineObj[0].benefit[skillIndex - 1].atkAndDef)
                        break
